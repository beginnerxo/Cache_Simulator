...
def runtrace(s, E, b, trace) -> Tuple[bool, List[int], List[int]]:
    ...
    cmd = f"./csim -s {s} -E {E} -b {b} -t {trace} > /dev/null"
    os.system("rm -rf .csim_results")
    status = os.system(cmd)
    if status != 0:
